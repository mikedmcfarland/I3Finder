...
object I3Finder{
	...
	val argParser = new OptionParser[Config]("I3Finder"){
		...
		opt[Unit](  'm',"move")				optional() action {(x,c)=>c.copy(move=true)}		 text("grab element and move it to current workspace")
		opt[String]('d',"dmenu") 			optional() action {(x,c)=>c.copy(dmenu=x)}			 text("arguments to pass along to demnu")
		opt[String]('w',"workspace-prefix") optional() action {(x,c)=>c.copy(workspacePrefix=x)} text("workspace displayname prefix")
